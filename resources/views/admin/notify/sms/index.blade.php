...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active font-size-12" aria-current="page">پیامک ها</li>
        </ol>
    </nav>
        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom">
                    ...
                    <a href="{{ route('admin.notify.sms.create') }}" class="btn btn-info btn-sm ">ایجاد اطلاعیه پیامکی جدید</a>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($sms as $key =>$value)
                            ...
                            <tr>
                                ...
                                <td>{{ $value->title }}</td>
                                <td>{{ $value->body }}</td>
                                <td>{{ jalaliDate($value->published_at) }}</td>
                                <td>
                                    <label>
                                        <input id="{{ $value->id }}" onchange="changeStatus({{ $value->id }})" data-url="{{ route('admin.notify.sms.status', $value->id) }}" type="checkbox" @if ($value->status === 1)
                                            checked
                                            @endif>
                                    </label>
                                </td>
                                <td class="width-16-rem" style="text-align: left">
                                    <a href="{{ route('admin.notify.sms.edit', $value->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit ms-1"></i>ویرایش</a>
                                    <form class="d-inline" action="{{ route('admin.notify.sms.destroy', $value->id) }}" method="post">
                                        @csrf
                                        {{ method_field('delete') }}

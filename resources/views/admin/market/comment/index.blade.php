                        ...
                        @foreach($comments as $key => $comment)
                            <tr>
                                ...
                                <td class="width-16-rem" style="text-align: left">
                                    <a href="{{ route('admin.market.comment.show', $comment->id) }}" class="btn btn-info btn-sm"><i class="fa fa-eye ms-1"></i>نمایش</a>
                                    @if($comment->approved == 0)
                                        <form class="d-inline" action="{{ route('admin.market.comment.approved', $comment->id) }}" method="post">
                                            @csrf
                                            {{ method_field('put') }}
